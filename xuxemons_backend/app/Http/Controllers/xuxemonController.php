...
class xuxemonController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $xuxemon->nivel = 0;
            $xuxemon->chuches = 0;
            $xuxemon->requisitos_crecimiento = 0;
    ...
    public function update(Request $request, $xuxemon_id)
{
    try {
        // Validar los datos del formulario
        $request->validate([
        ...
        // Buscar el Xuxemon por su ID
        $xuxemon = xuxemons::findOrFail($xuxemon_id);
        ...
        $xuxemon->update([
            ...
            'bajon_azucar' => $xuxemon->bajon_azucar,
            'sobredosis_azucar'=> $xuxemon->sobredosis_azucar,
            'atracon'=>$xuxemon->atracon,
            'nivel'=>$xuxemon->nivel,
            'chuches'=>$xuxemon->chuches,
            'requisitos_crecimiento'=>$xuxemon->requisitos_crecimiento
        ]);

            return response()->json(['message' => 'Xuxemon actualizado correctamente'], 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Ha ocurrido un error al actualizar el Xuxemon' . $e->getMessage()], 404);
        }
    }


    public function destroy(Request $request, $xuxemonId)
    {
        try {
            // Buscar el Xuxemon por su ID
